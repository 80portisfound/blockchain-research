# 🔗 Blockchain Research

> Personal research repository on blockchain technology, tokenization, and decentralized finance.

## 👋 About

3rd-year Computer Science student at Ajou University, exploring blockchain as a new financial paradigm.  
Focused on the intersection of **financial systems** and **decentralized technology**.

## 📚 Research Papers

| Title | Topic | Status |
|-------|-------|--------|
| [ERC-20 vs ERC-1400: Why Security Tokens Need a Different Standard](./ERC20-vs-ERC1400/README.md) | Token Standards | 🚧 In Progress |

## 🎯 Research Focus

- Token standards and security token offerings (STO)
- Decentralized identity (DID) and verifiable credentials
- Smart contract security and auditing
- Blockchain as financial infrastructure

## 📬 Contact

- GitHub: [@80portisfound](https://github.com/80portisfound)
